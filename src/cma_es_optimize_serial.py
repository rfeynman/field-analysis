#!/usr/bin/env python3
"""
CMA-ES Geometry Optimization Driver
Author: Erdong Wang
Mar.10 2026

This script performs geometry optimization using the CMA-ES (Covariance Matrix
Adaptation Evolution Strategy) algorithm. The goal is to minimize the high-order
multipole error (Sum_{n>2}) produced by the field analysis code.

The optimizer modifies electrode geometry parameters in a YAML template file,
runs the field analysis script for each candidate geometry, and evaluates the
objective function based on the returned multipole metrics.

------------------------------------------------------------------------------
WHAT THIS SCRIPT DOES
------------------------------------------------------------------------------

1) Reads a template geometry YAML file.
2) Extracts optimization variables (electrode parameters).
3) Runs CMA-ES to generate candidate geometries.
4) For each candidate:
      - Writes a temporary YAML file
      - Calls the field analysis script
      - Parses:
            Sum_{n>1}
            Uniformity radius
      - Uses Sum_{n>2} as the objective to minimize.
5) Saves all evaluations and outputs.
6) Copies the best geometry and output files to marked *_best files.
...
--template
    Geometry YAML template file.
    Default: electrode_geometry.yaml

--analysis
    Python field analysis script.
    Default: electrodes_field_analysis.py

The analysis script must:
    - Accept the geometry YAML file path as its argument.
    - Print lines containing:
        "Sum_{n>2} = <value>"
        "Uniformity radius = <value>"
    so this optimizer can parse them.

Optional inputs:

--outdir
    Name prefix of output directory.
    Default: opt_runs

--budget
    Maximum number of total evaluations.
    Default: 200
...
--python
    Python executable used to run the analysis script.
...
All results are saved under:
    ...
    PROJECT_ROOT/outputs/<outdir>_<timestamp>/

Inside this directory:

runs/
    genXXXX_evalYYYYYY.yaml
    genXXXX_evalYYYYYY.out.txt
    (all evaluated geometries and their outputs)

summary_<timestamp>.dat
    Tab-separated summary file containing:
        generation index
        evaluation index
        Sum_{n>2}
        uniformity radius
...
Basic usage:
    ...
    python cma_es_optimize_final.py
...
This represents the summed magnitude of higher-order multipole components
(sextupole, octupole, etc.), which correspond to nonlinear field errors.

Lower values indicate improved field quality.
...
Install missing dependency with:

    pip install cma pyyaml

------------------------------------------------------------------------------
NOTES
------------------------------------------------------------------------------

- This version uses rejection sampling to enforce geometry constraints.
- Bounds are automatically generated from the template values.
- The optimization stops when:
      - CMA-ES convergence condition is met, or
